...
def get_data(type_model):
    data = pd.read_csv(f'regression/optimization/opt_time/data/physical/time_400/optimum_values.csv')
    output = f'adhesivity_time_400'
    data_pred, model1 = get_data_opt_adhesivity_plots(data, name_model = f'time_400',type_model = type_model)
...
ax[0].scatter(filtered_data['weight_coefficient'], filtered_data[f'adhesivity_time_400'], 
...
ax[0].set_xlabel(r'$n$')
ax[0].legend()
save_figure(fig,'regression/optimization/opt_time/plots/actual_vs_ml')
plt.show()
