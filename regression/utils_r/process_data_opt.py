...
def data_throughput(throughput:float, data: pd.DataFrame) -> pd.DataFrame:
    ...
    Parameters:
    ----------
    throughput (float): the total volume we want to output
    ...
    data.loc[filter_finished_indices, f'efficiency_throughput_{throughput}'] = np.nan
    for index in filter_working_indices:
        ...
        if row['adhesivity'] == 0.0:
            data.at[index, f'efficiency_throughput_{throughput}'] = 0.0
        else:
            data.at[index, f'efficiency_throughput_{throughput}'] = 1 - (interp_func_flux_out(time_throughput)) / throughput if interp_func_flux_out is not None else np.nan
...
def data_time(time:int, data: pd.DataFrame) -> pd.DataFrame:
    ...
    data.loc[filter_finished_indices,f'volume_time_{time}'] = data.loc[filter_finished_indices, 'lifetime'] #/ time
    data.loc[filter_finished_indices, f'efficiency_time_{time}'] = data.loc[filter_finished_indices, 'efficiency'] 
    data.loc[filter_finished_indices, f'time_time_{time}'] = data.loc[filter_finished_indices, 'termination_time']
    data.loc[filter_working_indices, f'time_time_{time}'] = 400
    for index in filter_working_indices:
        row = data.loc[index]
        ...
        if row['adhesivity'] == 0.0:
                    ...
                    data.at[index, f'volume_time_{time}'] = 1
