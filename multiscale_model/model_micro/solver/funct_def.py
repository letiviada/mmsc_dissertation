...
def find_adhesivity(alpha: float,G: np.ndarray,delta: np.ndarray,l: float) -> float:
    ...
    integrand =  - alpha * G * delta_4 * (1-np.heaviside(-G*delta_4, 1.0))
    ...
    for r0 in range(R):
        for r1 in range(R):
                        # Get depo and perm
                        # -----
            depo_inte_5[:,:,r0,r1] = adhe_4[:,:,r0,r1]*G[:,:,r0,r1]*(-delta[:,:,r0])*(1-heav_4[:,:,r0])
                        # TODO: Check heav definition and indexing
    ...
    depo_1 = np.sum(a=depo_2, axis=0) # sum over i
        ...
        # Multiply by prefactors
        # ----------------------
    depo_1 = -(1/np.prod(leng_1))*depo_1
